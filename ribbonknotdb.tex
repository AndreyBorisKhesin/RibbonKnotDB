...
\cite{one}).
For each of these ribbon knots, we present a 4-strand tangle of which a specific
closure results in the original knot, while a second such closure results in the
2-component untangle.
We show that a tangle satisfying similar conditions exists for any ribbon knot
and that any knot corresponding to such a tangle is ribbon knot.
We also provide diagrams and the tangle information of these tangles for the
...
It has been shown that the 21 ribbon knots with 10 crossings or fewer can be
expressed as \textit{symmetric unions} (see \cite{many} and \cite{one}).
A symmetric union is a particular kind of knot diagram.
Such a diagram must have a central axis of symmetry, an axis on either side of
which the knot diagram is symmetric.
There are two structures that can be located on this axis.
The first are bridges, strands that approach the axis, cross it, and then keep
going, the way an actual bridge passes right over a river.
The second such set of structure are crossings, where one of the knot's
crossings is located directly on the axis, resulting in a slight asymmetry.
These symmetric unions can be used to construct presentations of these ribbon
knots that will be shown later.
...
A singularity occurs when the images two curves in the disc, the one whose
boundary is the knot, overlap and form a single curve in the image.
As the disc is immersed, the endpoints of the curve in the image must lie on the
knot.
We consider the points of the knot which coincide with these endpoints.
If the preimages of these points on the knot are the two endpoints of one of our
original curves, the singularity is deemed ribbon.
If each of the preimages is one of the endpoints of a different curve, the
singularity is deemed clasp.
...
A knot is the boundary of a disc immersed in three-dimensional space.
...
A \textit{ribbon knot} is a knot that is the boundary of an embedded disc that
contains only ribbon singularities (see \figSingularities).

Instead of immersing a single disc in three-dimensional space, we can immerse
several such discs, each connected to the next by an arc, resulting in what
looks a string of very flat beads.
After applying our immersion, we can replace each of the arc with a ribbon.
All this means is that we replace each of these strands with a pair of strands
that run right next to each other, each tracing out a path right next to where
the arc used to be.
This results in a single knot, but it has some interesting properties.
...
The ribbon connecting them is constructed by taking the image of a strand
connecting the discs and unzipping it, replacing a single strand with a pair.
Since the only intersections in ribbon presentations are between the ribbons
and the components, any ribbon presentation is a presentation of a ribbon knot,
making the name appropriate.}

A \textit{ribbon presentation} is a knot diagram which consists of components
and ribbons, satisfying certain properties.
We represent the components as a sequence of large rectangles.
The ribbons are pairs of parallel strands which connect each component to the
next.
They may intersect any of the components, but are not allowed to go through
other ribbons.
The ribbons are to appear as though they using to be a single arc, connecting
one component to the next, that was unzipped into a pair of strands after the
diagram was constructed.
If a knot diagram satisfies these properties, it is a ribbon presentation (see
\figPresentation).

If we have a ribbon presentation of a knot, by filling in the image of the full
disc and not just its boundary, we would see that all of the singularities will
occur when ribbons pass through the large rectangular components.
Thus, all of those singularities will be ribbon singularities so any knot that
has a ribbon presentation is a ribbon knot, making the name appropriate.
...
The central T represents a knot that shows that the dashed strands are tangled,
...
merely a 1-tangle whose two ends have been connected.

A \textit{closure} of a tangle is a way of stitching some the ends of the tangle
together in pairs.
For each pair we join, we are reducing the resulting number of strands by 1.
...
\end{paperthm}
\begin{proof}
We begin by proving the backward direction.
First we show that there exist surfaces bounded by $(\beta\circ\tau)(T)$ that do
not have any singularities.
We know that $(\beta\circ\tau)(T)$ results in the $n$-component unlink.
...
result in the boundaries of the discs being deformed to end our unlink, while
the surfaces will get continuously deformed and end up somewhere in space.
The key point is that since the surfaces of the original discs did not intersect
each other, after deforming them, they still will not.
...
side.
We we now see, is that by doing this, we have effectively constructed $\phi(T)$
with the added stitching $(t_1,~t_{2n})$ (see \figProof).
What this means is that all that we have left to show is that all of the
singularities that we added when we created and unzipped our arcs were ribbon
singularities.
This is due to the fact that we are trying to show that the knot that we
constructed, $\phi(T)$ with the stitching $(t_1,~t_{2n})$, is a ribbon knot.
...
immersed discs will turn into a ribbon singularity (see \figSingularities).
Thus, we have shown that there is a way to immerse a disc into three dimensional
space such that its boundary is $K$ and its image only contains ribbon
singularities.
Therefore, $K$ is a ribbon knot.
...
T' that can be closed along the bottom to result in T.}

\begin{paperlem}{Tangles}
For any $n$-strand tangle $T$ with all $2n$ of its ends placed along the top,
there exists a $2n$-strand tangle $T'$ such that applying the bottom part of the
full closure results in $T$ (see \figLemma).
\end{paperlem}

\begin{proof}[Proof of Lemma]
We know that since $T$ has $2n$ ends, $T$ has $n$ strands.
Each of these strands starts at one of the ends along the top, comes down,
weaves through the knot, and then comes out of the tangle through another one of
the ends along the top.
Each of these $n$ strands can be pulled down towards the bottom edge of the
tangle, resulting in what looks like $n$ hanging loops.
...
Therefore, this tangle given to us by \lemTangles satisfies the conditions we
want and we have shown its existence, proving the theorem.
\end{proof}
...
This allows us to find a tangle satisfying the conditions of \thmRibbon for each
of these 21 ribbon knots.
This is possible due to the fact that adjusting a few connections in a symmetric
union makes it untwist and untangle as we would like it to after applying the
top closure (see \figTop).
Surprisingly, each of these tangles only has 2 components, despite the fact that
